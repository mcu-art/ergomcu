...
extern "C"
{
	...
	/* Read is thread-safe and lock-free in a SPSC (Single Consumer, Single Producer) setup.
	ergo_uart doesn't support multiple readers (consumers). In case you
	need them, you shoul still read the data in a single task, and then share it
	...
	ergo_uart_writex_result_t ergo_uart_writex(ergo_uart_t *u, const void *src, uint32_t size);
	ergo_uart_writex_result_t ergo_uart_writex_part(ergo_uart_t *u, const void *src, uint32_t size);
