 ...
 * \param[in]       databuf: Pointer to memory to use as buffer data
 * \param[in]       size: Size of `databuf` in units of bytes
 ...
 * The maximum available size is always one byte less then the backing buffer capacity
 * due to the implementation limitation that forces readerIndex
 * to never reach writerIndex when there is unread data.
 * Otherwise, it'd be impossible to distinguish between
 * empty and full buffer without
 * introducing an additional variable, which may require
 * more advanced synchronization between threads.
 * \param[in]       b: Buffer handle
...
to avoid race conditions. */
static uint32_t _unread_size(uint32_t r, uint32_t w)
{
	return w - r;
}
 ...
 */
uint32_t ringbuf32_transfer(ringbuf32_t *src, ringbuf32_t *dest, uint32_t max_size)
{
...
void ringbuf32_trace(ringbuf32_t *b)
{
	...
	printf("\n  size: %d, free: %d, writerIndex: %d, readerIndex: %d\n",
		   b->size, free, b->writerIndex, b->readerIndex);
