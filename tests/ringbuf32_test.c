...
static char *TestCreation()
{
  uint8_t bdata[4];
  ringbuf32_t b;
  mu_assert("ring buffer must be properly initialized", initbuf(&b, bdata, 4) == 1);
  ...
  mu_assert("size field be properly initialized",
            b.size == 4);
...
static char *TestReadWrite()
{
  ...
  mu_assert("after 1 byte written, free space must be 3",
            3 == freeSize);

  mu_assert("after 1 byte written, unread data must be 1",
            1 == ringbuf32_unread_size(&b));

  mu_assert("after attempt to write more bytes than free space, operation must fail",
  ...
  uint8_t readBuf[4] = {0};
  mu_assert("1 byte read must be OK",
            1 == ringbuf32_read(&b, readBuf, 1));
  ...
  mu_assert("readBuf[0] must contain 1 (the first read byte)",
            1 == readBuf[0]);

  mu_assert("specifying more bytes to read than are available must read all available data",
  ...
  */
  ...
  mu_assert("writerIndex=bytesWritten",
            b.writerIndex == bytesWritten);

  mu_assert("readerIndex=bytesWritten",
            b.readerIndex == bytesWritten);
